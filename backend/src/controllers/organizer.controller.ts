...
class organizerController {
  private organizerRepo= AppDataSource.getRepository(Organizer);
    ...
    createOrganizer=async(req:Request<{},{},paymentInput>,res:Response<BaseResponse<any>>,next:NextFunction)=>{
      try {
        const {organizationName,organizerInfo}=req.body;
        if(!organizationName || !organizerInfo)
    ...
    getOneOrganizer=async(req:Request<{},{},paymentInput>,res:Response<BaseResponse<any>>,next:NextFunction)=> {
    try {
   ...
   updateOrganizer=async(req:Request<{},{},paymentInput>,res:Response<BaseResponse<any>>,next:NextFunction)=>{
    try {
      const {organizationName,organizerInfo}=req.body
      const organizerId = parseInt((req.params as any).organizerId, 10);
      const organizer = await this.organizerRepo.findOne({
